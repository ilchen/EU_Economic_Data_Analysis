...
class BankROEPBPlotter:
    ...
    def __init__(self, ticker_to_name: dict[str, str] | None = None, metrics=None):
        """
    ...
    def plot(self, roe_pb_df, roe_cutoff=None, pb_cutoff=None,
        ...
        """
        Plots Price-to-Book vs ROE for US Banks.
        ...
        title = "Price to Book vs ROE of European Banks"
    ...
    def print_cost_of_equity(self, roe_pb_df, slope, g, additional_beta=False):
        ...
        if self.metrics and additional_beta:
            rfr = metrics.riskless_rate.iloc[-1]
            coe = roe_pb_df.index.to_series().map(
                lambda ticker: (metrics.tickers.tickers[ticker].info.get("beta")
                    if (beta := metrics.tickers.tickers[ticker].info.get("beta")) is not None and beta >= 0.1
                    else metrics.get_beta([ticker]) ) * 0.05 + rfr).mean()
